data<-read.table("C:/Users/Manav Srinivasan/Desktop/household_power_consumption.txt",header=TRUE,sep=";",dec=".",stringsAsFactors=FALSE)
data<-data[data$Date %in% c("1/2/2007","2/2/2007"),]
datetime<-strptime(paste(data$Date,data$Time, sep=" "), "%d/%m/%Y %H:%M:%S")
globalact<-as.numeric(data$Global_active_power)
subm1<-as.numeric(data$Sub_metering_1)
subm2<-as.numeric(data$Sub_metering_2)
subm3<-as.numeric(data$Sub_metering_3)
grp<-as.numeric(data$Global_reactive_power)
png("plot4.png",width=480,height=480)
par(mfrow=c(2,2),mar=c(4,4,2,1),oma=c(0,0,2,0))
plot(datetime,globalact,type="l",xlab="",ylab="Global Active Power(kilowatts)")
volt<-as.numeric(data$Voltage)
plot(datetime,volt,type="l",ylab="Voltage")
plot(datetime,subm1,type="l",xlab="",ylab="Energy Submetering")
lines(datetime,subm2,type="l",col="red")
lines(datetime,subm3,type="l",col="blue")
legend("topright",c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),lty=1,lwd=2.5,bty="n",col=c("black","red","blue"))
plot(datetime,grp,ylab="Global_reactive_power",type="l")
dev.off()